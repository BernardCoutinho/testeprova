package br.com.tech4me.testeprova.Controller;
...
import br.com.tech4me.testeprova.Service.MusicaService;
...
public class MusicaController {
    ...
    @GetMapping(value = "/{id}")
    /*public Musica obterPorId(@PathVariable String id){
    ...
    @PostMapping
    ResponseEntity<MusicaDTO> adicionarMusicas(@RequestBody @Valid Musica musica){
        return new ResponseEntity<>(servico.adcMusica(musica), HttpStatus.CREATED);
    }
    ...
    @PutMapping(value = "/{id}")
    public ResponseEntity<MusicaDTO> atualizarMusica(@PathVariable String id, @RequestBody @Valid Musica musicaAtt){
        return new ResponseEntity<>(servico.atualizarMusica(id, musicaAtt), HttpStatus.OK);
    }
